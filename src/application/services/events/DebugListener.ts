...
export class DebugListener implements IEventListener {
  ...
  onTestSuiteLoaded(payload: PayloadByEvent[ContestEvents.TestSuiteLoaded]) {
    try {
      this.logger.debug(
        `Loaded test suite: ${payload.testSuite.name} (in ${JSON.stringify(payload.container)})`,
      );
    } catch (err) {
      console.log({ payload });
    }
  ...
  onHookRegistered(payload: PayloadByEvent[ContestEvents.HookRegistered]) {
    this.logger.debug(
      `Registered hook: ${payload.hook} in ${payload.testSuite.name}`,
    );
  }

  onTestLoaded(payload: PayloadByEvent[ContestEvents.TestLoaded]) {
    this.logger.debug(
      `Loaded test: ${payload.test.name} (in ${payload.testSuite.name}`,
    );
  }
  ...
  onTestEnded(payload: PayloadByEvent[ContestEvents.TestEnded]) {
    this.logger.debug(
      `Ended test: ${payload.test.name} with status ${JSON.stringify(payload.status)}`,
    );
  }
  ...
  onTestSuiteEnded(payload: PayloadByEvent[ContestEvents.TestSuiteEnded]) {
    this.logger.debug(
      `Ended test suite: ${payload.testSuite.name} with status ${JSON.stringify(payload.status)}`,
    );
  }
  ...
  onTestRunEnded(payload: PayloadByEvent[ContestEvents.TestRunEnded]) {
    this.logger.debug(
      `Ended test run with status ${JSON.stringify(payload.status)}`,
    );
  }
  ...
  onTestFileEnded(payload: PayloadByEvent[ContestEvents.TestFileEnded]): void {
    this.logger.debug(
      `Ended test file: ${payload.testFile.path} with status ${JSON.stringify(payload.status)}`,
    );
  }
}
