...
describe<{
  ...
  beforeEach(() => {
    executionContext = new ExecutionContext<SampleContext>(
      context.get("storages"),
    );
  });

  describe("set", () => {
    it("throws a ContextSetInTestLoop error", () => {
      expect(() => executionContext.set("name", "whatever")).toThrow(
        new ContextSetInTestLoop(),
      );
    });
  });
  ...
  describe("get", () => {
    describe("when the key is not set in the provided ContextStorage", () => {
      context.set("storages", [new Storage()]);

      it("throws a ContextNotSetError", () => {
        expect(() => executionContext.get("name")).toThrow(
          new ContextNotSetError("name"),
        );
      });
    });
