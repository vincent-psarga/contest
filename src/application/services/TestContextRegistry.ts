...
export class TestContextRegistry implements ITestContextRegistry {
  private readonly contextByTestContainerId = new Map<
    string,
    Context<unknown>
  >();
  private readonly storageByTestContainerId = new Map<
    string,
    Storage<unknown>
  >();
  ...
  set<T, K extends keyof T>(key: K, value: T[K]): void {
    ...
    const storage = (this.storageByTestContainerId.get(testContainer.id) ??
      new Storage()) as Storage<T>;
