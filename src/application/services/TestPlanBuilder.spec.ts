...
describe<{testContainers: ITestContainer[], defaultTimeout: number}>('TestPlanBuilder', (context) => {
    ...
    describe('#buildTestPlan', () => {
        context.when('test containers are empty', {testContainers: []}, () => {
            it('returns an empty TestPlan', () => {
                expect(testPlanBuilder.buildTestPlan(context.get('testContainers'))).toEqual([]);
            });
        });

        context.when('test containers are provided', {
            testContainers: [
                ITestFileFactory({
                    path: 'path/to/firstFile.spec.ts',
                    tests: [ITestFactory({name: 'First test'}), ITestFactory({name: 'Second test'})]
                }),
                ITestFileFactory({
            ...
            it('returns a line for each test', () => {
                ...
                expect(testPlanBuilder.buildTestPlan(testContainers)).toEqual(expected);
            })
        });

        context.when('test containers are imbricated', {
            testContainers: [
            ...
            it('properly builds test plan and respects the hierarchy', () => {
                ...
                const expected: TestPlan = [
                    ...
                    },
                    {
                        ...
                        ancestors: [testContainers[0]!, testContainers[0]!.testContainers[0]!],
                        only: false,
                        skip: false,
                        timeout: context.get('defaultTimeout'),
                    }
        ...
        context.when('when tests or container specify .only', {
            testContainers: [
                ITestContainerFactory({
        ...
        context.when('when tests or container specify .skip', {
            testContainers: [
                ITestContainerFactory({
        ...
        context.when('when tests or container specify a timeout', {
            testContainers: [
                ITestContainerFactory({
