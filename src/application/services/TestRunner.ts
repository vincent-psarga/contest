...
export class TestRunner implements ITestRunner {
  ...
  async runTestPlan(testPlan: TestPlan): Promise<TestSuiteStatus> {
    ...
    for (const testPlanEntry of testPlan) {
      status = this.updateTestSuiteStatus(
        status,
        await this.runTestPlanEntry(testPlanEntry),
      );
    }
  ...
  private async getTimeoutPromise(
    testPlanEntry: TestPlanEntry,
  ): Promise<TestStatus> {
    return new Promise<TestStatus>((resolve) =>
      setTimeout(() => {
        resolve({
          status: StatusEnum.fail,
          error: new TimeoutExceededError(
            testPlanEntry.test.name,
  ...
  private async executeTestPlanEntry({
    test,
    ancestors,
    skip,
  }: TestPlanEntry): Promise<TestStatus> {
    if (skip) {
  ...
  private updateTestSuiteStatus(
    current: TestSuiteStatus | undefined,
    newTestStatus: TestStatus | TestSuiteStatus,
  ): TestSuiteStatus {
    if (!current) {
      if (newTestStatus.status === StatusEnum.fail) {
        return {
          status: newTestStatus.status,
          errors: isTestStatus(newTestStatus)
            ? [newTestStatus.error]
            : newTestStatus.errors,
        };
      }
