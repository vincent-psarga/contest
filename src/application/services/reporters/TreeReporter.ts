...
export class TreeReporter extends AbstractReporter {
  ...
  onTestSuiteStarted(
    payload: PayloadByEvent[ContestEvents.TestSuiteStarted],
  ): void {
  ...
  onTestEnded(payload: PayloadByEvent[ContestEvents.TestEnded]) {
    ...
    switch (payload.status.status) {
      case StatusEnum.ok:
        this.logger.log(
          `${this.indent()}  ${this.logger.typo.success("✓")} ${payload.test.name}`,
        );
        return;
      case StatusEnum.fail:
        this.logger.log(
          `${this.indent()}  ${this.logger.typo.error("⨯")} ${payload.test.name}`,
        );
        return;
      case StatusEnum.notRun:
        this.logger.log(
          `${this.indent()}  ${this.logger.typo.faded("-")} ${payload.test.name}`,
        );
        return;
    }
  }
  ...
  private indent(): string {
    return Array.from(new Array(this.indentation))
      .map(() => "  ")
      .join("");
  }
}
