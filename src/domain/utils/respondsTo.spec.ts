...
describe<{ tbd: unknown }>('respondsTo', (context) => {
    ...
    describe('when tbd is an object', () => {
        context.when('when tbd does not have the expected property', { tbd: {}}, () => {
            it('returns false', () => {
                expect(respondsTo(context.get('tbd'), 'whatever')).toBe(false);
            })
        });

        context.when('when tbd does has the property but is not a function', { tbd: {whatever: true}},  () => {
            it('returns false', () => {
                expect(respondsTo(context.get('tbd'), 'whatever')).toBe(false);
            })
        });

        context.when('when tbd does has the property as a function', { tbd: {whatever: () => false}}, () => {
            it('returns true', () => {
